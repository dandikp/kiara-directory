...
const DivisionTable = ({
  data,
  total,
}: DatatableResponseReturn<SafeDivisionType>) => {
  const { pagination } = useDataTablePagination();
  const columns = useMemo<ColumnDef<SafeDivisionType>[]>(
    () => [
      {
        accessorKey: "id",
        meta: {
      ...
      },
      {
        ...
        header: ({ column }) => (
          <DatatableColumnHeader column={column} title="Kode Divisi" />
        ),
        cell: ({ row }) => (
