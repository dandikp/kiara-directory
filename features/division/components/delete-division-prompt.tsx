...
import { deleteDivisionById } from "../services/division.service";

const DeleteFieldPrompt = ({ data }: { data: SafeDivisionType }) => {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();

  const handleDelete = async () => {
    startTransition(() => {
      const deletePromise = deleteDivisionById(data.id);
