...
import { upsertDepartment } from "../services/department.service";
import type {
  DepartmentFormType,
  SafeDepartmentType,
...
interface DepartmentFormProps {
  data?: SafeDepartmentType;
}

const DepartmentForm = ({ data }: DepartmentFormProps) => {
  ...
  const onSubmitHandler = async (values: DepartmentFormType) => {
    let id: string | number = "";

    try {
      setIsPending(true);
      id = toast.loading("Mohon tunggu, sedang memproses...");
      const response = await upsertDepartment(values, data?.id);
